...
# BEGIN cross-guard (auto-inserted)
    # Allow host build even if rustup is missing; require rustup for cross targets
    if command -v rustup >/dev/null 2>&1; then
      if ! rustup target list --installed | awk '{print $1}' | grep -qx "\$t"; then
        host=$(host_triple)
        if [[ "\$t" != "\$host" ]]; then
          warn "target \$t not installed; skipping (hint: rustup target add \$t)"
          continue
        fi
      fi
    else
      host=$(host_triple)
      if [[ "\$t" != "\$host" ]]; then
        warn "rustup not found; skipping cross target \$t (install rustup or set CROSS_TARGETS=host)"
        continue
      fi
    fi

    # Apple Silicon -> Intel: require Intel Homebrew pkg-config and set vars
    if [[ "$(uname -s)/$(uname -m)" == "Darwin/arm64" && "\$t" == "x86_64-apple-darwin" ]]; then
      if [[ ! -x /usr/local/bin/pkg-config ]]; then
        warn "Intel pkg-config (/usr/local/bin/pkg-config) not found; skipping \$t. Install Intel Homebrew (+ glib/gstreamer)."
        continue
      fi
...
for c in "${chunks[@]}"; do
  case "$c" in
    ...
    universal) chunk_universal ;;
    homebrew)  chunk_homebrew ;;
    docker)    chunk_docker ;;
    clean)     chunk_clean ;;
    all)       chunk_build; chunk_smoke; chunk_package; chunk_universal; chunk_homebrew ;;
    *) die "unknown chunk: $c" ;;
  esac
done
