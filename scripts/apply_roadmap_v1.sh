...
[[ -f "$ROOT/Cargo.toml" ]] || { echo "!! run from repo root (Cargo.toml not found)"; exit 1; }

log "Writing upgraded mmx-cli/src/main.rs"
cp -a "$ROOT/mmx-cli/src/main.rs" "$ROOT/mmx-cli/src/main.rs.bak.$(date +%s)" 2>/dev/null || true
cat > "$ROOT/mmx-cli/src/main.rs" <<'RS'
...
log "Ensuring fixture video (fixtures/short.mp4)"
if [[ ! -f "$ROOT/fixtures/short.mp4" ]]; then
  if command -v ffmpeg >/dev/null 2>&1; then
    ffmpeg -v error -f lavfi -i testsrc=size=320x240:rate=30 -t 3 -pix_fmt yuv420p -y "$ROOT/fixtures/short.mp4" || warn "could not create fixture via ffmpeg"
  else
    warn "ffmpeg not found; skipping fixture creation"
  fi
fi
...
log "Building (gst feature)…"
cargo build -p mmx-cli -F mmx-core/gst --release

log "Doctor (JSON)…"
./target/release/mmx doctor --json || true

if [[ -f "$ROOT/fixtures/short.mp4" ]]; then
  log "Remux smoke…"
  ./target/release/mmx remux --input fixtures/short.mp4 --output /tmp/out_copy.mp4 --ss 0 --to 1 --stream-map "0:v:0,0:a?,0:s?"
fi

log "Done. Upgrade applied."
